...
io.read_file_by_newline("day1.txt", add_to_expense_report)
...
# Nice brute force O^3..... Yikes
break_flag = False
for i in range(0, len(expense_report)):
    for j in range(i, len(expense_report)):
        for k in range (j, len(expense_report)):
            if expense_report[i] + expense_report[j] +expense_report [k]== 2020:
                print("Answer: {}".format(expense_report[i]* expense_report[j] *expense_report[k]))
                break_flag = True
                break
    if break_flag:
        break